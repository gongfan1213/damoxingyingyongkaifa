...
#    ENV_NAME="RAG"
    ...
    set -x
    echo "启动命令是scripts/base_run.sh -s "$S" -w "$workers" -m "$milvus_port" -q "$qanything_port" -c -o -b "$openai_api_base" -k "$openai_api_key" "
    bash scripts/base_run.sh -s "$S" -w "$workers" -m "$milvus_port" -q "$qanything_port" -c -o -b "$openai_api_base" -k "$openai_api_key"
    set +x
    set +e
    if [ $? -ne 0 ]; then
...
echo "$CONDA_PREFIX"
echo "CONDA_INSTALL_PATH_2/envs/RAG"
# 检查激活是否成功
if [ "$CONDA_PREFIX" != "$CONDA_INSTALL_PATH_2/envs/RAG" ]; then
...
# 使用 Conda 环境运行脚本
echo "Executing the script for OpenAI API in the 'RAG' environment."
# 判断操作系统
if [ "$(uname)" = "Linux" ]; then
    S="LinuxOrWSL"
else
